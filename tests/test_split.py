...
def test_split_basic(tmp_path: Path):
    ...
    result = runner.invoke(
        main,
        [str(tmp_path), "--split", "50", "--outfile", str(tmp_path / "out.txt"), "--no-stdout"],
    )
...
def test_split_multiple_chunks(tmp_path: Path):
    ...
    result = runner.invoke(
        main,
        [str(tmp_path), "--split", "500", "--outfile", str(tmp_path / "out.txt"), "--no-stdout"],
    )
    assert result.exit_code == 0
...
def test_split_single_large_file(tmp_path: Path):
    ...
    result = runner.invoke(
        main,
        [str(tmp_path), "--split", "100", "--outfile", str(tmp_path / "out.txt"), "--no-stdout"],
    )
    assert result.exit_code == 0
